...
const mockCollection: MockCollection = {
  find: jest.fn<MockCollection["find"]>().mockReturnThis(), // .find() chains into .toArray()
  toArray: jest.fn(),
  insertOne: jest.fn(),
  findOne: jest.fn(),
  updateOne: jest.fn(),
...
describe("SetCards API", () => {
  ...
  it("POST /api/setcards - should return 400 when name or description are missing", async () => {
    const responce = await request(app).post("/api/setcards").send({
  ...
  it("GET /api/setcards/:setId -should return a specific set by ID", async () => {
    ...
    const response = await request(app).get(
      `/api/setcards/${fakeSetId.toString()}`,
    );
    expect(response.status).toBe(200);
